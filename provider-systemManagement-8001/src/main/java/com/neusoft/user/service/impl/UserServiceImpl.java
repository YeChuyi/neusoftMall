...
import com.neusoft.util.CreateMD5;
...
public class UserServiceImpl implements UserService {
     ...
     */
    @Override
    public AppResponse getUserByLogin(UserInfoVO userInfo) throws UnsupportedEncodingException{
        String pwd = CreateMD5.getMd5(userInfo.getUserPwd());
        userInfo.setUserPwd(pwd);
        List<UserInfo> users = userDao.getUserByLogin(userInfo);
        String token = "";
        if(null!=users&&1==users.size()){
            token = UUIDUtil.uuidStr();
            System.out.println("token"+token);
            redisUtils.LoginSet(token+"_id",users.get(0).getUserUuid(),loginTimeOut);
            redisUtils.LoginSet(token+"_account",users.get(0).getUserAccount(),loginTimeOut);
            redisUtils.LoginSet(token+"_name",users.get(0).getUserName(),loginTimeOut);
        }else{
            return AppResponse.bizError("登录失败！用户名或者密码错误");
        }
        return AppResponse.success("登录成功", token);
    }
